...
-- Email is retained so a vote can be verified and the voter can be told when
...
CREATE TABLE IF NOT EXISTS public.city_guide_votes (
  ...
  country_name TEXT,
  region       TEXT CHECK (region IN ('Europe', 'Asia', 'USA') OR region IS NULL),
  is_custom    BOOLEAN NOT NULL DEFAULT FALSE,
  email        TEXT NOT NULL,
  ip_hash      TEXT NOT NULL,
  anon_id      TEXT,
  created_at   TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  CHECK ((is_custom AND country_name IS NULL AND region IS NULL)
    OR (NOT is_custom AND country_name IS NOT NULL AND region IS NOT NULL))
);
